...
import es.grupo.apirestCRM.repository.IncidenciaRepository;
...
public class DashboardService {
    ...
    // --- NUEVO: Incidencias activas del usuario ---
    public List<Incidencia> getIncidenciasActivas(Usuario usuario) {
        // Asumiendo que el repositorio tiene un método para buscar por usuario y
        // estado,
        // o que la búsqueda ya está optimizada en el repositorio.
        // Ejemplo teórico: incidenciaRepository.findByClienteUsuarioAndEstado(usuario,
        // Estado.ABIERTA);
        // Por simplicidad, se trae todo y se filtra si es necesario.
        return incidenciaRepository.findByClienteUsuario(usuario).stream()
                .filter(i -> i.getEstado() != null && i.getEstado().name().equals("ABIERTA")) // Filtrar por estado
                                                                                              // 'ABIERTA' o similar
